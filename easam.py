...
class Easam(nn.Module):
    ...
    def forward(
        ...
        if len(pt[0].shape) == 3:
            ...
            masks = F.interpolate(low_res_masks, (self.image_encoder.img_size, self.image_encoder.img_size), mode="bilinear", align_corners=False) # (b, 1, 256, 256)
            outputs = {"low_res_logits": low_res_masks,
                       "masks": masks,
                       "edge_pred0": pred0,
                       "edge_pred1": pred1,
                       "edge_pred2": pred2,
